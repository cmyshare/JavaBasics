...
/**
 * 任务8_2使用字符流设计记事本
 * 使用字符流与缓冲流设计，记事本新建、文件保存、文件打开
 * 区别：在读写文件方式上，使用FileReader、BufferedReader、FileWriter、BufferedWriter读写文件
 */
					...
					else if(result == JOptionPane.YES_OPTION)//选择保存
					{
						...
						if(bcf == JFileChooser.APPROVE_OPTION){
							try {
								...
								Logger.getLogger(Notebook2.class.getName()).log(Level.SEVERE, null, ex);
							}
							new Notebook2();//新建文本文件
						}
					}
				}
			}
