...
package chap5.task5_2.practice5_2.manager;

import java.util.Scanner;
import chap5.task5_2.practice5_2.bean.House;
import chap5.task5_2.practice5_2.dao.HouseDaoImpUseOfArrayList;



public class HouseManager {
	//业务处理对象作为成员变量
	HouseDaoImpUseOfArrayList hdia=new HouseDaoImpUseOfArrayList();

	public HouseManager() {
		//初始化
		hdia=new HouseDaoImpUseOfArrayList();
	}
...
//			int action = sc.nextInt();
	...
	public void addHouse() {
		...
		//3： 判断房源信息是否存在
		boolean flag=hdia.isExist(house);

		if(flag)
		{
			System.out.println("已经存在，不能添加");
		}else{
			boolean f=hdia.insertHouse(house);
			if(f)
			{
	...
	public void updateHouseInfo() {
		...
		House house= hdia.selectById(houseId);
		...
		//4：调用修改方法，提示修改成功与
		boolean flag=hdia.updateHouse(houseId,house);
		if(flag)
		{
