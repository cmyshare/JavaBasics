...
package chap5.task5_1.practice5_1.manager;

import java.util.Iterator;
import java.util.List;
import java.util.Scanner;
import chap5.task5_1.practice5_1.bean.House;
import chap5.task5_1.practice5_1.dao.HouseDaoImpUseOfCollection;

//测试类HouseManager
public class HouseManager {
	//业务处理对象作为成员变量-调用接口实现层DaoImp
	HouseDaoImpUseOfCollection hdic=new HouseDaoImpUseOfCollection();

	public HouseManager() {
		//初始化
		hdic=new HouseDaoImpUseOfCollection();
	}

	public static void main(String[] args) {
		//实例化测试类HouseManager，自动调用无参构造。
	...
	public void showAction() {
		System.out.println("开始功能操作！ 请继续选择操作类型：");
		Scanner sc = new Scanner(System.in);
		quit: while (true) {
			//1判断输入值内容类型抛异常用try catch方法
			//2用instanceof关键字
			//3目标数据类型.class.isInstance(判断数据类型)
			int action = 0;
			try {
				action = sc.nextInt();
			} catch (Exception e) {
			...
			switch (action) {
				...
				case 4: //4：删除房源信息
					...
					break;
				case 5: // 5：房租价格
					System.out.println("房租价格");
					break;
				case 6: //6：房源状态
					System.out.println("房源状态");
					break;
				case 7: //7：小区名称
					System.out.println("小区名称");
					break;
				case 8: //8： 房屋楼层
					System.out.println("房屋楼层");
					break;
				default:
					break;
			}
	...
	public void addHouse() {
		...
		//3： 判断房源信息是否存在
		boolean flag=hdic.isExist(house);

		if(flag)
		{
			System.out.println("已经存在，不能添加");
		}else{
			boolean f=hdic.insertHouse(house);
			if(f)
			{
...
//	    House house= hdic.selectById(houseId);
//
//		//3：根据查询结果执行修改和不修改操作
//		    //a：如果不存在，提示“该房源不存在！”；
...
//
//		//4：调用修改方法，提示修改成功与
//		boolean flag=hdic.updateHouse(house);
//		if(flag)
//		{
...
//
//		    //如果不存在，提示：该房源不存在！
//		    //如果存在，则显示该房源信息，并提示是否确定删除，如果确定，则调用删除业务方法
//
//		//3：如果确定删除，调用删除该房源业务方法
//
//	}
//
	// 查看所有房源信息
